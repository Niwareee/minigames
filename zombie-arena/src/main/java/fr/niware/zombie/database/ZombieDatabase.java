...
public class ZombieDatabase {
    ...
    private static final String TABLE_NAME = "zombie_arena";
    ...
    public void createIfNotExists(UUID uuid, String name) {
        String request = "SELECT kills, survival_time FROM " + ZombieDatabase.TABLE_NAME + " WHERE player_uuid=?";
    ...
    public void handleStop() {
        String request = "UPDATE " + ZombieDatabase.TABLE_NAME + " SET kills=?, survival_time=? WHERE player_uuid=?";
        this.cachePlayers.values().forEach(profile -> {
            ...
            this.sqlDatabase.executeQuery(request, statement -> {
                ...
                statement.setLong(2, profile.getSurvivalTime());
    ...
    public void handleStart() {
        this.sqlDatabase.update("CREATE TABLE IF NOT EXISTS " + ZombieDatabase.TABLE_NAME + " (id int(11) AUTO_INCREMENT NOT NULL UNIQUE, player_uuid VARCHAR(255) NOT NULL UNIQUE, player_name TEXT NOT NULL, kills INT NOT NULL DEFAULT '0', survival_time BIGINT NOT NULL DEFAULT '0')");

        String request = "SELECT player_uuid, player_name, kills, survival_time FROM " + ZombieDatabase.TABLE_NAME;
        this.sqlDatabase.executeQuery(request, statement -> {}, resultSet -> {
            while (resultSet.next()) {
                UUID uuid = UUID.fromString(resultSet.getString("player_uuid"));
                this.topPlayers.put(uuid, new ZombieProfil(uuid, resultSet.getString("player_name"), resultSet.getInt("kills"), resultSet.getLong("survival_time")));
            }
            return null;
        });
    ...
    public void addStats(ZombiePlayer zombiePlayer) {
        ...
        this.cachePlayers.get(zombiePlayer.getId()).addKills(zombiePlayer.getKills());
        this.cachePlayers.get(zombiePlayer.getId()).addSurvivalTime(zombiePlayer.getSurvivalTime());
    }
